...
class LivePortraitEngine:
    ...
    def _ensure_models_downloaded(self):
        ...
        if self.models_dir is None:
            ...
            try:
                self.models_dir = Path(snapshot_download(
    ...
    def detect_face(self, image_rgb):
        ...
        if self.bbox_model is None:
            ...
            if not insightface_model_path.exists():
                raise FileNotFoundError(f"InsightFace models not found in {self.models_dir}")

            self.bbox_model = FaceAnalysisDIY(name='buffalo_l', root=str(insightface_model_path), allowed_modules=['detection'])
    ...
    def _create_pipeline(self):
        ...
        inference_cfg = InferenceConfig(
            ...
            checkpoint_S=str(retargeting_models_dir / "stitching_retargeting_module.pth")
        )
    ...
    def prepare_source(self, source_image):
        ...
        mask_ori = cv2.warpAffine(self.get_mask(), crop_trans_m, get_rgb_size(img_rgb), cv2.INTER_LINEAR)
        ...
        return PreparedSrcImg(img_rgb, crop_trans_m, x_s_info, f_s_user, x_s_user, mask_ori)

    def calc_fe(self, x_d_new, eyes, eyebrow, wink, pupil_x, pupil_y, mouth, eee, woo, smile,
    ...
    def edit_expression(self, image: Image.Image, expression_params: dict) -> Image.Image:
